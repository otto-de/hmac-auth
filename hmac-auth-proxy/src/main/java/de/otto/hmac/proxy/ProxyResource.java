...
public class ProxyResource {
    ...
    public String getRequest(@Context UriInfo uriInfo, @Context Request request) {
        return createBuilder(uriInfo, request).get(String.class);
    }

    @Path("{resource:.*}")
    @POST
    public String postRequest(String body, @Context UriInfo uriInfo, @Context Request request) {
        return createBuilder(uriInfo, request, body).post(String.class);
    }

    @Path("{resource:.*}")
    @PUT
    public String putRequest(String body, @Context UriInfo uriInfo, @Context Request request) {
        return createBuilder(uriInfo, request, body).put(String.class);
    }
    ...
    public String deleteRequest(@Context UriInfo uriInfo, @Context Request request) {
        return createBuilder(uriInfo, request).delete(String.class);
    }

    @Path("{resource:.*}")
    @HEAD
    public ClientResponse headRequest(@Context UriInfo uriInfo, @Context Request request) {
        return createBuilder(uriInfo, request).head();
    }

    @Path("{resource:.*}")
    @OPTIONS
    public String optionsRequest(@Context UriInfo uriInfo, @Context Request request) {
        return createBuilder(uriInfo, request).options(String.class);
    }


    private WebResource.Builder createBuilder(UriInfo uriInfo, Request request, String body) {
        URI targetUri = withTargetHostAndPort(uriInfo.getRequestUriBuilder());
        return webResourceWithAuth(body, request.getMethod(), targetUri);
    }

    private WebResource.Builder createBuilder(UriInfo uriInfo, Request request) {
        return createBuilder(uriInfo, request, "");
    }
